...
class TestSyntax < Test::Unit::TestCase
  def test_comment
    ...
    assert_equal :cyan, segs[0][2]
  end

  def test_keyword
    segs = Rvim::Syntax.highlight('def foo', :ruby)
    keyword = segs.find { |_, _, c| c == :magenta }
  ...
  def test_string_dominates_keyword
    segs = Rvim::Syntax.highlight('"def foo"', :ruby)
    string = segs.find { |_, _, c| c == :green }
  ...
  def test_symbol
    ...
    sym = segs.find { |_, _, c| c == :yellow }
    assert_not_nil sym
  end

  def test_constant
    segs = Rvim::Syntax.highlight('class Foo', :ruby)
    const = segs.find { |_, _, c| c == :blue }
